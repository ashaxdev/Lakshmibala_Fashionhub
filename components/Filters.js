...
export default function Filters({ activeSize, onSizeChange, sort, onSortChange }) {
  return (
      ...
      <div className="flex items-center gap-2 overflow-x-auto no-scrollbar">
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs font-medium border shrink-0 ${
            !activeSize ? 'bg-brand-pink text-white border-brand-pink' : 'border-brand-ink/15 text-brand-ink/70'
          }`}
        >
          All
        </button>
        {SIZE_OPTIONS.map((s) => (
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium border shrink-0 ${
              activeSize === s ? 'bg-brand-pink text-white border-brand-pink' : 'border-brand-ink/15 text-brand-ink/70'
            }`}
          >
            {s}
          </button>
